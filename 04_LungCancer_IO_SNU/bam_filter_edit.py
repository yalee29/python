...
if __name__ == '__main__':
    ...
    parser.add_argument('-w', '--window', help="window", default=5000) #100000 -> 10000 -> 5000
    ...
    for f in args.files:
        ...
        if args.out is not None:
            ...
            bam_out = pysam.AlignmentFile(os.path.splitext(f)[0] + '_filtered_.bam', "wb", template=bam_in)


##간단한 filtering 조건을 통과하면서 다른 chromosome에 align 되지 않는 read들은 바로 output bamfile에 넣어줌.

        for chro in [str(i) for i in range(1,23)] + ['X', 'Y']:
            #chro = "1" #test용
            for start in list(range(0, chr_length[chro], args.window)): #fetch 별 test: start 0 -> 121400000(chr1)/ 33100000(chr2)

                #print(chro+"_"+str(start), datetime.datetime.now())
                ...
                #count reads ----------------------------------------------------------------------------------
                counter =  0
                for rd in bam_in.fetch(chro, start, start+args.window):
                    counter += 1
                    if(counter > 50000):
                        break
                print(chro+"_"+str(start)+"_"+str(counter),datetime.datetime.now())
