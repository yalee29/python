...
res_dic={}
total_sigs=[]
info_list=subprocess.Popen('ls |grep INFO.tsv$', stdout=subprocess.PIPE, shell = True)
for line in info_list.stdout:
	sampleid=line.split('.INFO.tsv')[0]
	if sampleid not in res_dic.keys():
		res_dic[sampleid]={}
	info_file=open(line.rstrip())
	info_line=info_file.readline().strip()
	cs=round(float(info_line.split('similarity: ')[1]),2)
	res_dic[sampleid]['cs']=cs

sig_list=subprocess.Popen('ls |grep signature_exposures.tsv$', stdout=subprocess.PIPE, shell = True)
for line in sig_list.stdout:
	sampleid = line.split('.signature_exposures')[0]
	res_dic[sampleid]['proportion']={}
	sig_file=open(line.rstrip())
...
spec_list=subprocess.Popen('ls |grep signature_spectra.tsv$', stdout=subprocess.PIPE, shell = True)
for line in spec_list.stdout:
	sampleid = line.split('.signature_spectra')[0]
	spec_file=open(line.rstrip())
